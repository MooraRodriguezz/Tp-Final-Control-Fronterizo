...
public class PersistenciaGestorUsuarios {
    ...
    public static void GuardarUsuarios(ObservableList<Usuario>listaDeUsuarios){
        List<Oficial> oficiales = listaDeUsuarios.stream().filter(u -> u instanceof Oficial).map(u ->(Oficial) u).collect(Collectors.toList());
        List<Administrador> administradores = listaDeUsuarios.stream().filter(u -> u instanceof Administrador).map(u ->(Administrador) u).collect(Collectors.toList());
